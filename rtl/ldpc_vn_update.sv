...
(
    ...
    input  ldpc_pkg::llr_t channel_llr	[ldpc_pkg::N],
    input  ldpc_pkg::llr_t cn_to_vn		[ldpc_pkg::M][ldpc_pkg::MAX_CN_DEG],
    output ldpc_pkg::llr_t vn_to_cn		[ldpc_pkg::M][ldpc_pkg::MAX_CN_DEG],
    output ldpc_pkg::llr_t app_llr		[ldpc_pkg::N]
);
    ...
    always_ff @(posedge clk or posedge rst) begin
        ...
        if(rst) begin
            ...
            if(start) begin
                // APP LLR
                for(vn=0;vn<N;vn=vn+1) begin
                    sum = channel_llr[vn];
                    for(k=0;k<VN_DEGREE[vn];k=k+1) begin
                        cn = VN_CONN[vn][k];
                ...
                for(vn=0;vn<N;vn=vn+1) begin
                    ...
                    for(k=0;k<VN_DEGREE[vn];k=k+1) begin
                        ...
                        edge = edge_index(cn,vn);
                        sum = channel_llr[vn];
                        for(int j=0;j<VN_DEGREE[vn];j=j+1) begin
                            if(j!=k) begin
                                edge2 = edge_index(VN_CONN[vn][j],vn);
                                sum += cn_to_vn[VN_CONN[vn][j]][edge2];
                            end
                        end
